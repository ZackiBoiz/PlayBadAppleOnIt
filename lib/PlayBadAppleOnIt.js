...
class PlayBadAppleOnIt extends EventEmitter {
    ...
    constructor(options = {}) {
        ...
        if (/^https?:\/\//.test(options.file)) {
            throw new TypeError("constructor only accepts local file paths. Use PlayBadAppleOnIt.create(options) to pass remote URLs.");
        }
    ...
    static async create(options = {}) {
        ...
        if (/^https?:\/\//.test(filePath)) {
            if (typeof fetch !== "function") {
                throw new Error("Global fetch not available. Node >=18 / Bun is required to download remote files.");
            }

            const tmpPath = path.join(
                os.tmpdir(),
                "play-bad-apple-on-it-" + Date.now() + "-" + Math.random().toString(36).slice(2) + path.extname(new URL(filePath).pathname)
            );
        ...
        const merged = { ...options, file: filePath };
        const inst = new PlayBadAppleOnIt(merged);
        if (cleanupTmp) inst.#cleanupTmpFile = cleanupTmp;
        return inst;
    }
    ...
    get fps() { return this.#regulatedFps; }
    set fps(v) {
    ...
    #maybeCleanupTmp() {
        if (this.#cleanupTmpFile) {
            try { fs.unlinkSync(this.#cleanupTmpFile); } catch (e) { /* ignore */ }
            this.#cleanupTmpFile = null;
        }
    }
